...
| P2 | 🟢 达标话题超过 30 天未复测（🕐 已过期） | `mock <话题>` 复测防衰减 |
...
### 自评校准
从能力画像「自评 vs 教练评分」表汇总：偏差均值 <±X> → 判定 <持续高估 / 持续低估 / 校准良好>。
- 持续高估 → 面后自我感觉不可作为决策依据，以教练评分为准
- 持续低估 → 警惕悲观误判导致提前放弃（对照管线中实际通过的轮次）

### 模拟 vs 真实漂移
对比 `data/mock-records/progress.md` 的模拟评分与真实面试结果（管线通过/终止 + 记录表现）：
- 某话题模拟达标/高分，但真实面试连续 2+ 场失分 → **评分虚高**：在能力画像该话题标注「按 -1 档采信」，收紧后续模拟中该话题的评分口径
- 模拟评分低但真实面试屡屡通过 → 评分过严或临场发挥型：降低焦虑级别，把准备时间挪给真正的缺口
- 漂移结论写入能力画像「模拟 vs 真实结果」表（日期 | 漂移结论 | 校准动作）
```
